...
export let MARVISUR_SEDES = (marvisurSedesData as any[]).map(s => ({
  ...s,
  _s: _normSede(`${s.n} ${s.dist} ${s.prov} ${s.dep} ${s.addr}`),
}));

export function searchMarvisur(q: string, lim = 14) {
  const norm = (str: string) => (str || "").toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9 ]/g," ").replace(/\s+/g," ").trim();
  const nq = norm(q);
  if (!nq) return [];
  const terms = nq.split(" ").filter(t => t.length >= 2);
...
export function findNearestMarvisur(lat: number, lon: number, limit = 3): { sede: typeof MARVISUR_SEDES[0]; distKm: number }[] {
  return MARVISUR_SEDES
    .map(s => ({ sede: s, distKm: calcularDistancia(lat, lon, s.lat, s.lon) }))
    .sort((a, b) => a.distKm - b.distKm)
    .slice(0, limit);
}
